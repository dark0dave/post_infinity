...
mod tests {
    ...
    fn valid_simple_creature_file_header_parsed() {
        ...
        assert_eq!(header.signature, "CRE ".to_string());
        assert_eq!(header.version, "V1.0".to_string());
        assert_eq!(header.base_hp, 8);
        assert_eq!(header.level_first_class, 1);
        assert_eq!(header.level_second_class, 1);
        assert_eq!(header.level_third_class, 1);
        assert_eq!(header.sex, 1);
        assert_eq!(header.strength, 9);
        assert_eq!(header.strength_bonus, 0);
        assert_eq!(header.intelligence, 9);
        assert_eq!(header.wisdom, 9);
        assert_eq!(header.dexterity, 9);
        assert_eq!(header.constitution, 9);
        assert_eq!(header.charisma, 9);
        assert_eq!(header.morale, 10);
        assert_eq!(header.offset_to_item_slots, 996);
        assert_eq!(header.offset_to_memorized_spell_table, 996);
        assert_eq!(header.dialog_ref, "dbeggar\0".to_string());
    }
    ...
    fn valid_creature_file_header_parsed() {
        ...
        let mut reader = Cursor::new(&buffer);
        let creature: BGEECreatureHeader = reader.read_le().unwrap();
